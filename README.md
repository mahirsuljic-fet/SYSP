Sistemsko programiranje (SYSP)
==============================

# Sadržaj
- [Materijal za učenje](#materijal-za-ucenje)
  - [Literatura](#literatura)
  - [Materijal sa fakulteta](#FET)
  - [YouTube](#youtube)



# Materijal za učenje


### [Literatura](./Literatura/)
- [Unix Systems Programming - Communication, Concurrency, and Threads](<./Literatura/Unix Systems Programming - Communication, Concurrency, and Threads.pdf>)
- [The Linux Programming Interface](<./Literatura/The Linux Programming Interface.pdf>)
- [C++ Concurrency in Action](<./Literatura/C++ Concurrency in Action.pdf>)


### FET

#### [**Predavanja**](./Predavanja)
- [P1 - Uvod u fajl sisteme](./Predavanja/p1.md)
- [P2 - Direktorije, hard linkovi i putanje](./Predavanja/p2.md)
- [P3 - Login, shell komande, permisije, autorizacija i eksalacija privilegija](./Predavanja/p3.md)
- [P4 - `sudo`, set gid bit, sticky bit, default permisije, načini izvršavanja skripti, fajl sistem kernel tabele, `init` fajl deskriptori](./Predavanja/p4.md)

#### [~Prezentacije~](./Prezentacije)
...

#### [~Vježbe~](./Vjezbe)
...

#### [~Zadaće~](./Zadace)
...

### YouTube
- [Unix/Linux Files Explained - what is an inode? (hardlinks, symlinks, etc.)](https://www.youtube.com/watch?v=ScDv02ff8oc)
